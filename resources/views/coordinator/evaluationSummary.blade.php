          ...
          <ul class="mt-6">
            <li class="relative px-6 py-3">
              ...
              <a
                ...
                href="{{route('coordinator_dashboard')}}"
              >
                <svg
                  class="w-5 h-5"
                  aria-hidden="true"
  ...
  logoutBtn.onclick = function() {
   window.location.href="{{route('login.index')}}";
        ...
        <main class="h-full overflow-y-auto">
          ...
          @php
        $teachers = App\Models\EvaluationResult::where('evaluator_type', 'Coordinator')
    ->where('evaluator_id', session('user_id'))
    ->distinct()
    ->pluck('teacher_id');
          ...
          @php
          $teach= App\Models\Teacher::where('id', $teacher)->first();
            $fullname= $teach->teacher_first_name. " ". substr($teach->teacher_middle_name, 0, 1). ". ". $teach->teacher_last_name. " ". $teach->teacher_suffix;

            $scores= App\Models\EvaluationResult::where('teacher_id', $teacher)->where('evaluator_type', 'Coordinator')->where('evaluator_id', session('user_id'))->get();
            $num=1;
            @endphp
            <div class="section">
                <details>
                  <summary class="dark:text-gray-200">{{$fullname}}</summary>
                  <table class="w-full">
                    <thead class="border border-solid-gray-800">
                        <th class="border border-solid-gray-800 dark:text-gray-200">Question</th>
                        <th class="border border-solid-gray-800 dark:text-gray-200">Scores</th>
                    </thead>
                    <tbody>
               ...
               <tr>
                <td class="border border-solid-gray-800 dark:text-gray-200 text-sm">{{$num. ". ". App\Models\Question::where('id', $score->question_id)->first()->question_content;}}</td>
                <td class="border border-solid-gray-800 dark:text-gray-200">{{$score->evaluation_score}}</td>
               </tr>
                  ...
                  <tr>
                    <td class="border border-solid-gray-800 dark:text-gray-200 text-sm">Evaluation Remarks</td>
                    <td class="border border-solid-gray-800 dark:text-gray-200">{{$score->evaluation_remarks}}</td>
                   </tr>
