...
<html :class="{ 'theme-dark': dark }" x-data="data()" lang="en">
  ...
  logoutBtn.onclick = function() {
   ...
   $sections= App\Models\ClassSchedule::where('sched_class_name', $student->student_section)->where('sched_day', date('l'))->get();
  ...
  $strandname= App\Models\Strand::where('id', $strand->strand_id)->first()->strand_name;
   ...
   @endphp
        <main class="h-full overflow-y-auto">
          ...
          >
            Class Schedule of {{$strand->year_level}} - {{$strand->section}} {{$strandname}}<br> Current Day: {{date('l')}}
            </h2>
