    ...
    >
      ...
      <div class="flex flex-col flex-1 w-full">
        ...
        @php
         ...
         $middle_initial= substr($teacher->teacher_middle_name, 0, 1);
         $fullname= $teacher->teacher_first_name . " ". $middle_initial. ". ". $teacher->teacher_last_name. " ". $teacher->teacher_suffix;
        @endphp
          <h2
          class="my-6 text-2xl font-semibold text-gray-700 dark:text-gray-200"
        >Class Schedule of Ms/Mr. {{$fullname}}
     </h2>
     @php 
     $schedTeach= App\Models\ClassSchedule::where('sched_teacher', $id)->get();
   @endphp
     <div class="w-full mb-8 overflow-hidden rounded-lg shadow-xs">
        <div class="w-full overflow-x-auto">
          <table class="w-full whitespace-no-wrap">
            <thead>
              ...
              >
                <th class="px-4 py-3">Class Room</th>
            ...
            >
            @foreach($schedTeach as $SchedTeachers)
            <tr class="text-gray-700 dark:text-gray-400">
                <td class="px-4 py-3 text-sm">
               {{App\Models\Room::where('id',$SchedTeachers->sched_room)->first()->room_name}}
              </td>
              <td class="px-4 py-3 text-sm">
             {{$SchedTeachers->sched_day}}
            </td>
            <td class="px-4 py-3 text-sm">
         {{$SchedTeachers->sched_time}}
          </td>
          <td class="px-4 py-3 text-sm">
          {{App\Models\AssignedSubject::where('id', $SchedTeachers->sched_subject)->first()->assigned_subject}}
        </td>
        <td class="px-4 py-3 text-sm">
        {{$SchedTeachers->sched_class_name}}
      </td>
                </tr>
            @endforeach
            </tbody>
        ...
        $schedRooms= App\Models\ClassSchedule::where('sched_room', $id)->get();
        @endphp
        <h2
        class="my-6 text-2xl font-semibold text-gray-700 dark:text-gray-200"
      >Class Schedule of  {{$room->room_name}}</h2>
      <div class="w-full mb-8 overflow-hidden rounded-lg shadow-xs">
        <div class="w-full overflow-x-auto">
          <table class="w-full whitespace-no-wrap">
            <thead>
            ...
            @foreach($schedRooms as $schedRoom)
            <tr class="text-gray-700 dark:text-gray-400">
                <td class="px-4 py-3 text-sm">
                    @php 
                   $teacherName= App\Models\Teacher::where('id', $schedRoom->sched_teacher)->first();
                   ...
                   $subject= App\Models\AssignedSubject::where('id', $schedRoom->sched_subject)->first();
            ...
            <td class="px-4 py-3 text-sm">
         {{$schedRoom->sched_time}}
          </td>
          <td class="px-4 py-3 text-sm">
       {{$subjectName}}
        </td>
        <td class="px-4 py-3 text-sm">
            {{$schedRoom->sched_class_name}}
      </td>
                </tr>
            @endforeach
        </tbody>
